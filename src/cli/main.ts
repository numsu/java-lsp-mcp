...
import { lenientStdin } from "../mcp/handshake.js";
...
export async function main(argv = process.argv.slice(2)): Promise<number> {
  ...
  if (command === "version") { const data = { name: application.name, version: application.version, mcpRevision: application.mcpRevision, sdk: application.sdkVersion, node: `external ${process.version}`, jdk: "external", jdtls: application.jdtlsVersion, compiler: "ecj" }; process.stdout.write(argv.includes("--json") ? `${JSON.stringify(data)}\n` : `${data.name} ${data.version} (MCP ${data.mcpRevision}, JDT LS ${data.jdtls}, ECJ)\n`); return 0; }
