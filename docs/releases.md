...
Verify an archive on a matching platform with `npm run smoke-release -- --bundle artifacts/java-lsp-mcp-<platform>.<zip|tar.gz>`; it extracts the archive and runs the bundled `version --json` launcher with a sanitized `PATH`.
